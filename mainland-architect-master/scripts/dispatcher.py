...
class MainlandArchitectDispatcher:
    def __init__(self):
        self.base_path = SKILL_ROOT
        self.sub_skills_base = os.path.join(self.base_path, "subskills")
        self.config_base = os.path.join(SCRIPT_DIR, "config")

        self.skill_file_overrides = {}

        self.skill_id_aliases = {
            "cn-fsd-licensing-compliance": "cn-fire-acceptance-closeout",
            "hk-fsd-licensing-compliance": "cn-fire-acceptance-closeout",
            "hk-fire-acceptance-closeout": "cn-fire-acceptance-closeout",
        }
    ...
    def _load_translation_assets(self):
        ...
        if not os.path.exists(map_path) or not os.path.exists(schema_path):
            self.translation_status = {
                "status": "warning",
                "message": "translation_map assets not found in scripts/config",
            }
            return
    ...
    def _normalize_skill_id(self, skill_id):
        if not skill_id:
            return None
        skill_id = self.skill_id_aliases.get(skill_id, skill_id)
